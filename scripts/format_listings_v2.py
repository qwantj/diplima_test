...
{m.group(0).lstrip()}"""
    ...
    # 5. Enhance variable declarations (careful regex to keep original assignments)
    # Match: newline, spaces, type, name, '=', expression, ';'
    def var_repl(m):
        indent = m.group(1)
        var_type = m.group(2)
        var_name = m.group(3)
        ...
        # Don't annotate return statements or plain assignments
    ...
    code = re.sub(r'(\n\s*)(int|double|float|bool|auto|std::string|uint64_t|size_t)\s+([a-zA-Z0-9_]+)\s*;', var_repl, code)

    # 6. Functional blocks start and end
    # Place end of block before return
    code = re.sub(r'(\n\s*)(return\s+.*?;)', r'\n\1// --- Конец функциональной части ---\1\2', code)
    
    # Place start of block after method signature
