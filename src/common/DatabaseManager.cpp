...
void DatabaseManager::flushEvents(QSqlDatabase& db) {
        ...
        if (!bufferedMessages.empty()) {
            ...
            for (const auto& msg : bufferedMessages) {
                try {
                    ...
                    q.bindValue(4, r.pps);
                    q.bindValue(5, (qint64)r.totalPackets);
    ...
    // Defer QSqlQuery initialization until we confirm db is open, else we get warnings.
    std::unique_ptr<QSqlQuery> qLive;
    bool livePrepared = false;
    ...
    while (eventQueue_.try_dequeue(entry)) {
        ...
        if (!livePrepared) {
            qLive = std::make_unique<QSqlQuery>(db);
            qLive->prepare("INSERT INTO events (session_id, timestamp, label, confidence, pps, total_packets, features, model_name) VALUES (?,?,?,?,?,?,?,?)");
            livePrepared = true;
        }

        qLive->bindValue(0, entry.result.sessionId);
        qLive->bindValue(1, entry.result.timestamp);
        qLive->bindValue(2, entry.result.label);
        qLive->bindValue(3, entry.result.confidence);
        qLive->bindValue(4, entry.result.pps);
        qLive->bindValue(5, (qint64)entry.result.totalPackets);
        nlohmann::json featJson = entry.result.features;
        qLive->bindValue(6, QString::fromStdString(featJson.dump()));
        qLive->bindValue(7, QString::fromStdString(entry.result.modelName));
        qLive->exec();
        ++count;
    }
...
void DatabaseManager::flushSnapshots(QSqlDatabase& db) {
    ...
    if (db.isOpen()) {
        ...
        // If DB isn't open, just drain the queue to avoid infinite growth
        while (snapshotQueue_.try_dequeue(entry)) {}
    }
    if (count > 0)
...
void DatabaseManager::flushSecurityEvents(QSqlDatabase& db) {
    ...
    if (db.isOpen()) {
        ...
        // If DB isn't open, just drain the queue
        while (securityEventQueue_.try_dequeue(entry)) {}
    }
    if (count > 0)
