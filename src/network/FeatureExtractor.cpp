 ...
 * @brief Модуль извлечения признаков сетевого трафика (реализация).
 *
 * Назначение: Реализация методов агрегации и вычисления признаков сетевых потоков.
 * Входные данные: Сырые пакеты, перехваченные с интерфейса.
 * Результаты: Нормализованные векторы признаков для классификатора.
 * Метод решения: Декапсуляция пакетов (L2-L4), ведение статистики по 5-tuple, расчет вторичных метрик.
...
/**
 * @brief Загрузка параметров масштабирования (μ, σ) из JSON.
 * @param jsonPath Путь к файлу конфигурации.
...
bool FeatureExtractor::loadScalerParams(const std::string& jsonPath) {
  try {
    ...
    if (!f.is_open()) {
      AppLogger::get()->error("FeatureExtractor: ошибка открытия файла скейлера: {}", jsonPath);
      return false;
    }
    ...
    AppLogger::get()->info("Загружены параметры масштабирования из {}", jsonPath);
    return true;
  } catch (const std::exception& e) {
    AppLogger::get()->error("FeatureExtractor: ошибка парсинга скейлера: {}", e.what());
    return false;
  }
}

// Обработка пакета из буфера
...
/**
 * @brief Основной метод разбора пакета и обновления статистики.
 * @param rawPacket Сырой пакет.
 */
void FeatureExtractor::processPacket(pcpp::RawPacket& rawPacket) {
  ...
  // Гистограмма размеров (5 бинов: 0-64, 65-256, 257-512, 513-1024, 1025+)
  ...
  // Определение портов и протокола для формирования 5-tuple
  ...
  // Анализ транспортного уровня (TCP)
  ...
  // Анализ транспортного уровня (UDP)
  ...
  // Формирование ключа потока (инвариантного к направлению)
  FlowKey fk;
  ...
  // Определение направления пакета
  ...
  // Обновление специфических счетчиков TCP
  if (isTcp) {
  ...
  // Сбор данных для анализа энтропии полезной нагрузки
...
/**
 * @brief Расчет векторов сырых признаков для всех активных потоков.
 */
std::vector<std::vector<double>> FeatureExtractor::computeFeaturesBatch() {
  ...
  for (const auto& [key, stats] : activeFlows_) {
    ...
    // Вычисление информационной энтропии Шеннона
    double entropy = 0.0;
    ...
    // Сборка вектора (согласно порядку, ожидаемому моделью)
...
/**
 * @brief Пакетная нормализация признаков.
 */
...
/**
 * @brief Расчет нормализованных признаков для интерфейса в целом (агрегировано).
 */
std::vector<double> FeatureExtractor::computeNormalizedFeatures() {
  ...
  // Суммирование статистик всех потоков
  for (const auto& [key, stats] : activeFlows_) {
...
/**
 * @brief Сбор данных телеметрии для отправки в монитор.
 */
void FeatureExtractor::fillTelemetry(DetectionResult& result) const {
  double deltaT = 1.0; // Фиксированное окно инференса в 1 сек.
  ...
  // Сортировка и выборка Top Talkers
  ...
  std::sort(ports.begin(), ports.end(), [](auto& a, auto& b) { return a.second > b.second; });
...
// Сброс всех счетчиков для нового окна
...
/**
 * @brief Автоматический подбор скейлера на основе имени модели.
 */
void FeatureExtractor::setModelScaler(const std::string& modelName) {
  namespace fs = std::filesystem;
  fs::path p(modelName);
  fs::path dir = p.parent_path();
  if (dir.empty()) dir = "models";

  std::string stem = p.stem().string();
  auto mpos = stem.find("_model");
  if (mpos != std::string::npos) stem = stem.substr(0, mpos);
