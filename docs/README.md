...
- активного противодействия атакам (блокировка IP);
...
| ddos_core | Статическая библиотека | Общее ядро: захват, признаки, инференс, протокол, БД, конфиг |

| ddos_collector | Консольный исполняемый файл | Сбор трафика, ML-классификация, запись в БД, TCP-сервер, блокировка |
...
- Извлечение 16 признаков окна (расширяемый CICIDS-совместимый вектор).
...
- Активная защита: модуль **FirewallManager** для блокировки атакующих IP.

- Управление конфигурацией через JSON (`config.json`).
...
1. Подготовьте `config.json` или используйте аргументы командной строки.

2. Запустите collector от имени администратора:
...
3. В отдельном окне запустите monitor:
...
4. Опционально сгенерируйте тестовую UDP-нагрузку:
...
- -m, --model <path> (по умолчанию из конфига или models/rf_model.onnx)
...
- --db-password <pass> (по умолчанию: пустая строка)
...
- Текущая реализация признаков агрегирует окно трафика на уровне интерфейса и 5-tuple потоков.
