...
**Принцип работы:** каждые 1 секунда (`INFERENCE_WINDOW_SEC`) собирается окно пакетов. По завершении окна вычисляется набор статистических признаков на основе потоко-ориентированной агрегации (flow-based statistics), которые затем нормализуются и передаются в `ModelInferencer::predict()`.
