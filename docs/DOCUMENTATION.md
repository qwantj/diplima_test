...
- **core/** — оркестрация детекции (inference window loop (1 s), управление блокировками (FirewallManager), отслеживание инцидентов.
...
4. Каждые 1 секунда вычисляется вектор из 8 нормализованных признаков.
...
|---|---|---|
| `stats` | каждые 1 сек | Полная телеметрия: pps, label, confidence, top IPs/ports, features, blocked IPs |
| `snapshot` | каждые 1 сек | Лёгкий снимок: pps, total_packets, current_label |
